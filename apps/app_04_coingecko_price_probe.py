...
def probe_coingecko_prices():
    """
    Connects to the CoinGecko public API to fetch prices for
    Bitcoin and Ethereum against USD and TWD, using the centralized api_client.
    """
    ids = "bitcoin,ethereum"
    vs_currencies = "usd,twd"
    url = f"https://api.coingecko.com/api/v3/simple/price?ids={ids}&vs_currencies={vs_currencies}"

    print(f"Executing CoinGecko Price Probe...\nTarget URL: {url}\n")

    try:
        data = fetch_json_data(url)
        if data:
            print("Status: HTTP/1.1 200 OK (assumed by successful fetch_json_data)\n")
            print("--- RAW JSON RESPONSE (Prices) ---")
        ...
        print(f"--- PRICE PROBE FAILED: An exception occurred ---")
        # Error details are already printed by fetch_json_data

if __name__ == "__main__":
    probe_coingecko_prices()
