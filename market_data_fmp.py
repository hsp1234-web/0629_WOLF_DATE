...
cache_session = requests_cache.CachedSession('fmp_cache', expire_after=timedelta(days=1), backend='sqlite')
...
cached_http = cache_session #requests_cache.CachedSession('fmp_cache', expire_after=timedelta(days=1), backend='sqlite', session=http)
...
def get_fmp_api_key():
    ...
    if not api_key:
        logger.error("API_KEY_FMP not found in environment variables. Ensure it's set (e.g., via .env file or Colab Secrets mapped by a runner script).")
        return None
    return api_key
...
def get_daily_price_data(api_key: str, symbol: str, start_date: str, end_date: str, max_retries=3, backoff_factor=1) -> pd.DataFrame | None:
    ...
    logger.info(f"Fetching daily price data for {symbol} from {start_date} to {end_date}")
    ...
    while current_retry < max_retries:
        try:
            ...
            if not data or "historical" not in data or not data["historical"]:
                logger.warning(f"No historical data found for {symbol} for the given date range.")
                return None

            df = pd.DataFrame(data["historical"])
            df = df.sort_values(by="date").reset_index(drop=True)
            logger.info(f"Successfully fetched {len(df)} rows of daily price data for {symbol}")
            return df

        except requests.exceptions.HTTPError as e:
            logger.error(f"HTTP error occurred: {e.response.status_code} - {e.response.text}")
            if e.response.status_code == 401: # Unauthorized
                logger.error("API key is invalid or expired. Please check your FMP_API_KEY.")
                return None
            if e.response.status_code == 404: # Not found
                logger.error(f"Symbol {symbol} not found or no data available for the given range.")
                return None
            if e.response.status_code == 429: # Too many requests
                logger.warning(f"Rate limit exceeded. Retrying in {backoff_factor * (2 ** current_retry)} seconds...")
            # For other HTTP errors, retry as per strategy
        except requests.exceptions.RequestException as e:
            logger.error(f"Request failed: {e}")
        except json.JSONDecodeError:
            logger.error("Failed to decode JSON response. The API might be down or returning malformed data.")
            # No specific retry for JSON decode error unless it's a symptom of a transient network issue handled by Retry

        current_retry += 1
        if current_retry < max_retries:
            time.sleep(backoff_factor * (2 ** (current_retry -1))) # Exponential backoff
        else:
            logger.error(f"Max retries reached for {symbol}. Unable to fetch data.")
            return None
    return None
...
def get_company_profile(api_key: str, symbol: str, cache_days=7) -> dict | None:
    ...
    try:
        ...
        with requests_cache.CachedSession(cache_name='fmp_cache', expire_after=timedelta(days=cache_days), backend='sqlite', session=http) as specific_session:
            ...
            profile_data = data[0] if isinstance(data, list) and len(data) > 0 else data
            logger.info(f"Successfully fetched profile data for {symbol}")
            return profile_data

    except requests.exceptions.HTTPError as e:
        logger.error(f"HTTP error occurred while fetching profile for {symbol}: {e.response.status_code} - {e.response.text}")
        ...
        logger.error(f"Request failed while fetching profile for {symbol}: {e}")
        return None
    except json.JSONDecodeError:
        logger.error(f"Failed to decode JSON response for {symbol} profile. API might be returning malformed data.")
        return None
...
def get_financial_statements(api_key: str, symbol: str, period="quarter", limit=5, cache_days=7) -> list | None:
    ...
    logger.info(f"Fetching {period} income statements for {symbol} (limit {limit})")

    try:
        with requests_cache.CachedSession(cache_name='fmp_cache', expire_after=timedelta(days=cache_days), backend='sqlite', session=http) as specific_session:
            response = specific_session.get(url, timeout=15) # Longer timeout for potentially larger data
            response.raise_for_status()

            data = response.json()
            if not data or not isinstance(data, list) or len(data) == 0:
                logger.warning(f"No {period} income statements found for {symbol} with limit {limit}.")
                return None

            logger.info(f"Successfully fetched {len(data)} {period} income statements for {symbol}")
            return data # FMP returns a list of statements

    except requests.exceptions.HTTPError as e:
        logger.error(f"HTTP error occurred while fetching financials for {symbol}: {e.response.status_code} - {e.response.text}")
        if e.response.status_code == 401:
            logger.error("API key is invalid or expired. Please check your FMP_API_KEY.")
        elif e.response.status_code == 404:
            logger.error(f"Financials for symbol {symbol} (period {period}) not found.")
        return None
    except requests.exceptions.RequestException as e:
        logger.error(f"Request failed while fetching financials for {symbol}: {e}")
        return None
    except json.JSONDecodeError:
        logger.error(f"Failed to decode JSON response for {symbol} financials. API might be returning malformed data.")
        return None

if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Fetch data from Financial Modeling Prep API.")
    parser.add_argument("symbol", help="Stock symbol (e.g., AAPL)")
    parser.add_argument("--start_date", help="Start date in YYYY-MM-DD format for price data")
    parser.add_argument("--end_date", help="End date in YYYY-MM-DD format for price data")
    parser.add_argument("--output_file", help="Path to save the output CSV/JSON file")
    parser.add_argument("--action", choices=['price', 'profile', 'financials'], default='price', help="Action: 'price', 'profile', 'financials'")
    parser.add_argument("--period", choices=['quarter', 'annual'], default='quarter', help="Period for financials: 'quarter' or 'annual'")
    ...
    if args.action == 'price':
        if args.start_date and args.end_date:
            df = get_daily_price_data(api_key, args.symbol, args.start_date, args.end_date)
            if df is not None:
                if args.output_file:
                    df.to_csv(args.output_file, index=False)
                    logger.info(f"Price data for {args.symbol} saved to {args.output_file}")
                else:
            ...
            df = get_daily_price_data(api_key, args.symbol, start_date_dt.strftime('%Y-%m-%d'), end_date_dt.strftime('%Y-%m-%d'))
            if df is not None:
                if args.output_file:
                    df.to_csv(args.output_file, index=False)
                    logger.info(f"Price data for {args.symbol} (last 30 days) saved to {args.output_file}")
                else:
                    # Output to stdout if no file is specified
                    print(df.to_csv(index=False))
        ...
        if profile_data:
            if args.output_file:
                ...
                logger.info(f"Profile data for {args.symbol} saved to {args.output_file}")
            else:
                # Output to stdout if no file is specified
        ...
        if args.symbol:
            financials_data = get_financial_statements(api_key, args.symbol, period=args.period, limit=args.limit)
            if financials_data:
                if args.output_file:
                    with open(args.output_file, 'w') as f:
                        json.dump(financials_data, f, indent=4)
                    logger.info(f"Financial statements for {args.symbol} (period: {args.period}, limit: {args.limit}) saved to {args.output_file}")
                else:
        ...
        logger.warning("Invalid action specified. Use 'price', 'profile', or 'financials'.")
        logger.info("For 'price': symbol, [start_date, end_date (optional, default last 30 days)]")
        logger.info("For 'profile': symbol")
        logger.info("For 'financials': symbol, [--period (quarter|annual, default quarter)], [--limit (int, default 5)]")
